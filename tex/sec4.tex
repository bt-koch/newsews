...
\end{tikzpicture}
%}
\caption{Process of Article to Sentiment Score}
\label{fig:pipeline}
\end{figure}
...
\section{Step 3: Construction of Sentiment Indicator} \label{sec:construction}
...
As a result, we get a weekly and a daily indicator which proxies the sentiment in news articles were $s_{b,t} \in [-1,1]$ and $\tilde{s}_{b,t} \in [-1,1]$. Values in $[-1,0)$ indicate negative sentiment, $0$ neutral sentiment and $(0,1]$ positive sentiment. Furthermore, we define an adjusted $N$-period backward-looking weighted moving average:\\
